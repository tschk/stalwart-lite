...
use crate::common::config::smtp::queue::QueueName;
use crate::common::ipc::QueueEvent;
use crate::common::{KV_LOCK_QUEUE_MESSAGE, Server};
use crate::smtp::queue::manager::{LockedMessage, Queue};
...
use crate::store::{Deserialize, IterateParams, Serialize, U64_LEN, ValueKey};
use crate::trc::{AddContext, ServerEvent, SpamEvent};
    ...
    async fn try_lock_event(&self, queue_id: QueueId, queue_name: QueueName) -> bool {
            ...
            .await
        {
            Ok(result) => {
                if !result {
                    crate::trc::event!(
                        Queue(crate::trc::QueueEvent::Locked),
                        QueueId = queue_id,
    ...
    pub async fn queue(
        ...
        let (flags, event, train_spam) = match source {
            MessageSource::Authenticated => (
                FROM_AUTHENTICATED,
                crate::trc::QueueEvent::QueueMessageAuthenticated,
                None,
            ),
            MessageSource::Unauthenticated {
                ...
                crate::trc::QueueEvent::QueueMessage,
                train_spam,
            ),
            MessageSource::Unauthenticated {
                dmarc_pass: false,
            ...
            MessageSource::Dsn => (FROM_DSN, crate::trc::QueueEvent::QueueDsn, None),
            MessageSource::Report => (FROM_REPORT, crate::trc::QueueEvent::QueueReport, None),
            MessageSource::Autogenerated => (
                FROM_AUTOGENERATED,
                crate::trc::QueueEvent::QueueAutogenerated,
                None,
            ),
        };
        ...
        crate::trc::event!(
            ...
            From = if !self.message.return_path.is_empty() {
                crate::trc::Value::String(self.message.return_path.as_ref().into())
            } else {
            ...
            To = self
                ...
                .recipients
                .iter()
                .map(|r| crate::trc::Value::String(r.address.as_ref().into()))
                .collect::<Vec<_>>(),
            ...
            NextDsn = self
                ...
                .next_dsn(None)
                .map(crate::trc::Value::Timestamp),
            Expires = self.message.expires(None).map(crate::trc::Value::Timestamp),
        );
        ...
        for (queue_name, due) in self.message.next_events() {
            batch.set(
                ValueClass::Queue(QueueClass::MessageEvent(crate::store::write::QueueEvent {
                    due,
            ...
            && let Some(config) = &server.core.spam.classifier
        {
            ...
            crate::trc::event!(
                ...
                Expires = crate::trc::Value::Timestamp(hold_period),
                SpanId = self.span_id,
            );
        }
                    ...
                    let mut cur_message = <Archive<AlignedBytes> as Deserialize>::deserialize(
                        bytes.ok_or_else(|| {
                            crate::trc::StoreEvent::NotFound
                                ...
                                .caused_by(crate::trc::location!())
                                .ctx(crate::trc::Key::QueueId, params.u64(0))
                        })?,
                    )
                        ...
                        && cur_message.recipients.len() == new_message.recipients.len()
                    {
                            ...
                            .filter(|(_, rcpt)| rcpt.queue.as_slice() == queue_name)
                        {
                            cur_message.recipients[rcpt_idx] =
                                rkyv_deserialize(rcpt).caused_by(crate::trc::location!())?;
                        }

...
use crate::services::task_manager::index::ReindexIndexTask;
use crate::store::{
            ...
            (Some("reindex"), Some(index), id, &Method::GET) => {
                ...
                let index = SearchIndex::try_from_str(index).ok_or_else(|| {
                    crate::trc::ResourceEvent::BadParameters
                        .reason("Invalid search index specified")
                })?;

                let jmap = self.clone();
                tokio::spawn(async move {
                    if let Err(err) = jmap.reindex(index, account_id, tenant_id).await {
                        crate::trc::error!(err.details("Failed to reindex FTS"));
                    }
                });
...
pub async fn recalculate_quota(server: &Server, account_id: u32) -> crate::trc::Result<()> {
    let mut quota = 0;
...
pub async fn destroy_account_blobs(server: &Server, account_id: u32) -> crate::trc::Result<()> {
    ...
    if has_data {
        ...
        for index in [
            ...
            if let Err(err) = server
                ...
                .await
            {
                crate::trc::error!(err.details("Failed to delete FTS index"));
            }
        }
    }
...
pub async fn reset_imap_uids(server: &Server, account_id: u32) -> crate::trc::Result<(u32, u32)> {

...
use crate::common::{Server, auth::AccessToken, scripts::plugins::PluginContext};
...
use crate::trc::{AddContext, SieveEvent};
use crate::types::{
    blob_hash::BlobHash,
...
pub trait SieveScriptIngest: Sync + Send {
    ...
    fn sieve_script_get_active(
        &self,
        account_id: u32,
    ) -> impl Future<Output = crate::trc::Result<Option<ActiveScript>>> + Send;

    fn sieve_script_get_by_name(
        &self,
        account_id: u32,
        name: &str,
    ) -> impl Future<Output = crate::trc::Result<Option<Sieve>>> + Send;

    fn sieve_script_compile(
        &self,
        account_id: u32,
        document_id: u32,
    ) -> impl Future<Output = crate::trc::Result<Option<CompiledScript>>> + Send;
}
...
impl SieveScriptIngest for Server {
    ...
    async fn sieve_script_ingest(
        ...
        active_script: ActiveScript,
        autogenerated: &mut Vec<AutogeneratedMessage>,
    ) -> crate::trc::Result<IngestedEmail> {
        // Parse message
        let message = if let Some(message) = MessageParser::new().parse(raw_message) {
            message
        } else {
            return Err(crate::trc::EventType::MessageIngest(
                        ...
                        if let Some(message) = messages.get(message_id) {
                            ...
                            if message.raw_message.len() <= self.core.jmap.mail_max_size {
                                crate::trc::event!(
                                    ...
                                    To = recipients
                                        .iter()
                                        .map(|r| crate::trc::Value::String(r.as_str().into()))
                                        .collect::<Vec<_>>(),
        ...
        if let Some(reject_reason) = reject_reason {
            Err(
                crate::trc::EventType::MessageIngest(crate::trc::MessageIngestEvent::Error)
                    .ctx(crate::trc::Key::Code, 571)
                    .ctx(crate::trc::Key::Reason, reject_reason),
            )
    ...
    async fn sieve_script_get_active_id(&self, account_id: u32) -> crate::trc::Result<Option<u32>> {
        self.store()
    ...
    async fn sieve_script_get_active(
        &self,
        account_id: u32,
    ) -> crate::trc::Result<Option<ActiveScript>> {
    ...
    async fn sieve_script_compile(
        ...
        account_id: u32,
        document_id: u32,
    ) -> crate::trc::Result<Option<CompiledScript>> {
        ...
        if let Some(script) = script_bytes.get(script_offset..).and_then(|bytes| {
            ...
            match self.core.sieve.untrusted_compiler.compile(
                ...
                Ok(sieve) => {
                    ...
                    let compiled_bytes = sieve.serialize().caused_by(crate::trc::location!())?;
                    ...
                    let mut new_script_object =
                        rkyv::deserialize(unarchived_script).caused_by(crate::trc::location!())?;
                    let blob_hash =
                    ...
                    batch
                        ...
                        .set(
                            SieveField::Archive,
                            new_archive.serialize().caused_by(crate::trc::location!())?,
                        )
                        .clear(BlobOp::Link {
                            hash: blob_hash,
                ...
                Err(error) => Err(crate::trc::StoreEvent::UnexpectedError
                    .caused_by(crate::trc::location!())
                    .reason(error)
